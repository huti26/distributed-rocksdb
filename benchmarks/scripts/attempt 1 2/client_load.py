...
# Create output dir
pathlib.Path(f"/media/ssd/drdb/benchmarks/varying-fieldlength/ssd/load/{ycsb_fieldlength}/").mkdir(exist_ok=True, parents=True)

commands = []
for i in range(0,4):
    ycsb_log_dir = f"drdb.log_dir=/media/ssd/drdb/logs/{i}"
    ycsb_output = f"/media/ssd/drdb/benchmarks/varying-fieldlength/ssd/load/{ycsb_fieldlength}/{i}"

    ycsb_insertstart = "insertstart=" + str(250000 * i)
    ycsb_insertcount = "insertcount=250000"
